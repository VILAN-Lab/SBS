...
class BaseModel(nn.Module):
    def __init__(self, w_emb, q_emb, v_att, q_net, v_net, classifier,c_1,c_2, relu):
    ...
    def forward(self, v, q, labels, bias, v_mask, loss_weight):
        ...
        if labels is not None:
            rubi_logits=logits*torch.sigmoid(q_pred) * loss_weight
...
def build_baseline0_newatt(dataset, num_hid):
    ...
    v_net = FCNet([dataset.v_dim, num_hid])
    q_net = weight_norm(nn.Linear(q_emb.num_hid, num_hid), dim=None)
    relu = nn.ReLU()
    ...
    return BaseModel(w_emb, q_emb, v_att, q_net, v_net, classifier,c_1,c_2, relu)