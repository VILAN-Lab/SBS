...
class IndexedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # * represent a copy of self.dataset
        return self.dataset[idx][0], self.dataset[idx][1], self.dataset[idx][2],\
               self.dataset[idx][3], self.dataset[idx][4], idx
...
def _load_dataset(dataroot, name, img_id2val, dataset):
    ...
    """
    if dataset=='cpv2':
        answer_train_path = os.path.join(dataroot, 'cp-cache', 'train_target.pkl')
        answer_test_path = os.path.join(dataroot, 'cp-cache', 'val_target.pkl')
        name = "train" if name == "train" else "test"
        question_train_path = os.path.join(dataroot, 'vqacp_v2_train_questions.json')
        question_test_path = os.path.join(dataroot, 'vqacp_v2_test_questions.json')
        with open(question_train_path) as f:
            questions_train = json.load(f)
        with open(question_test_path) as f:
      ...
      answer_train_path = os.path.join(dataroot, 'cache', '%s_target.pkl' % name)
      answer_test_path = os.path.join(dataroot, 'cache', '%s_target.pkl' % name)
      question_train_path = os.path.join(dataroot, 'v2_OpenEnded_mscoco_%s2014_questions.json' % name)
      question_val_path = os.path.join(dataroot, 'v2_OpenEnded_mscoco_%s2014_questions.json' % name)
      with open(question_train_path) as f:
        questions_train = json.load(f)["questions"]
      with open(question_val_path) as f:
        questions_test = json.load(f)["questions"]

    questions_train.sort(key=lambda x: x['question_id'])
    with open(answer_train_path, 'rb') as f:
        answers_train = cPickle.load(f)
    answers_train.sort(key=lambda x: x['question_id'])

    questions_test.sort(key=lambda x: x['question_id'])
    with open(answer_test_path, 'rb') as f:
        answers_test = cPickle.load(f)
    answers_test.sort(key=lambda x: x['question_id'])

    utils.assert_eq(len(questions_train), len(answers_train))
    utils.assert_eq(len(questions_test), len(answers_test))
    entries = []
    ...
    for question, answer in zip(questions_test, answers_test):
        if answer["labels"] is None:
            raise ValueError()
        utils.assert_eq(question['question_id'], answer['question_id'])
        utils.assert_eq(question['image_id'], answer['image_id'])
...
class VQAFeatureDataset_Conca(Dataset):
    def __init__(self, name, dictionary, dataroot='data', dataset='cpv2',
                 use_hdf5=False, cache_image_features=False, keepidx=None):
        super(VQAFeatureDataset_Conca, self).__init__()
        self.name=name
        if dataset=='cpv2':
            ...
            for entry in tqdm(self.entries, ncols=100, desc="caching-features"):
                ...
                if img_id not in image_to_fe:
                    if use_hdf5:
                        ...
                        fe = torch.load(open('data/rcnn_feature/' + str(img_id) + '.pth', 'rb'), encoding='iso-8859-1')[
                            'image_feature']
                        fe = torch.from_numpy(fe).view(36, 2048)
    ...
    def __getitem__(self, index):
        ...
        if self.keepidx is not None:
            entry = self.entries[self.keepidx[index]]
        else:
        ...
        if self.image_to_fe is not None:
            ...
            features = torch.load(open('data/rcnn_feature/' + str(entry["image_id"]) + '.pth', 'rb'), encoding='iso-8859-1')['image_feature']
            features = torch.from_numpy(features).view(36, 2048)
        ...
        if "bias" in entry:
            return features, ques, target, entry["bias"], 0
        else:
            return features, ques, target, 0, 0


    def __len__(self):
        if self.keepidx is not None:
            return len(self.keepidx)
        else:
